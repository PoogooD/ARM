...
begin
  ...
  try
    ...
    { Save word }
    WordApp.ActiveDocument.SaveAs('O:\бамашка.doc');
...
begin
  {sql_s2('select gr_');
  Word_StringReplace(
    ExtractFilePath(ParamStr(0))+'blanki\templates\Протокол.docx',
...
begin
 ...
 { sql_s('select comp_user_id as [cid] from comps where comp_ip='''+GetLocalIP+''' ');
  ...
  label40.Caption:='Добро пожаловать, '+#13+ADOQuery1.FieldValues['user_name']+' '+ ADOQuery1.FieldValues['user_ot'];
...
begin
  ...
  IF Edit30.Text='Поиск' Then Edit30.Text:='';
end;
...
begin
  LoadKeyboardLayout('00000419', KLF_ACTIVATE);
  IF Edit9.Text='Поиск' Then Edit9.Text:='';
end;

procedure TForm1.Edit7Enter(Sender: TObject);
begin
  LoadKeyboardLayout('00000419', KLF_ACTIVATE);
  IF Edit7.Text='Поиск' Then Edit7.Text:='';
end;

procedure TForm1.Edit7Exit(Sender: TObject);
begin
  IF Edit7.Text='' Then Edit7.Text:='Поиск';
end;

procedure TForm1.Edit8KeyPress(Sender: TObject; var Key: Char);
begin
  if (key=',') or (key='/') or (key='ю') then key:='.';
end;

procedure TForm1.Edit30Exit(Sender: TObject);
begin
  IF Edit30.Text='' Then Edit30.Text:='Поиск';
end;

procedure TForm1.Edit9Exit(Sender: TObject);
begin
  IF Edit9.Text='' Then Edit9.Text:='Поиск';
end;
...
var s1,s2,s3,role,role_test, tmp:string;
    i:SmallInt;
begin
  sql_s2('select st_fam + '' '' + LEFT(st_name, 1)+ ''.'' +LEFT(st_ot, 1) as [Имя], '+
  '              st_dr as [Дата рождения], '+
  '              st_ter as [Участок], '+
  '              st_prof as [Профессия], '+
  '              st_raz as [Разраяд], '+
  '              st_staj as [Стаж], '+
  '              st_obr as [Образования], '+
  '              st_ar as [Адрес], '+
  '              st_numpric as [№ приказа], '+
  '              st_dot as [Дата исключения], '+
  '              st_dza as [Дата зачисления] '+
...
begin
  ...
  edit19.Text:='0000';
  edit8.Text:='10.109.6';
  Button7.Caption:='Добавить';
  Button13.Caption:='Очистить';
end;
...
begin
  if gro_or_stud=0 then begin
    ...
    if ADOQuery1.FieldValues['pro_id']=null then MessageBox(Form1.HANDle, PChar('Не правильно указана программа'), PChar('Ошибка!'), MB_OK + MB_ICONERROR) else begin
      s1:=GetToken(ComboBox2.Text,'[',2);
      s2:=GetToken(s1,']',1);
      pid:=ADOQuery1.FieldValues['pro_id'];
      if CheckBox2.Checked=true then act:=1 else act:=0;
      if Button15.Caption='Создать' then begin
...
begin
  if ADOQuery3.FieldValues['Имя ПК']<>null then edit10.Text:=ADOQuery3.FieldValues['Имя ПК'] else edit10.text:='Не указано';
  if ADOQuery3.FieldValues['IP']<>null then edit8.Text:=ADOQuery3.FieldValues['IP'] else edit8.text:='Не указано';
  if ADOQuery3.FieldValues['Кабинет']<>null then edit18.Text:=ADOQuery3.FieldValues['Кабинет'] else edit18.text:='Не указано';
  if ADOQuery3.FieldValues['ШК ПК']<>null then edit19.Text:=ADOQuery3.FieldValues['ШК ПК'] else edit19.text:='Не указано';
  if ADOQuery3.FieldValues['Корпус']<>null then ComboBox1.Text:=ADOQuery3.FieldValues['Корпус'] else ComboBox1.text:='Не указано';
  if ADOQuery3.FieldValues['Тип ПК']<>null then edit20.Text:=ADOQuery3.FieldValues['Тип ПК'] else edit20.text:='Не указано';
  if ADOQuery3.FieldValues['IMEI ПК']<>null then edit21.Text:=ADOQuery3.FieldValues['IMEI ПК'] else edit21.text:='Не указано';
  edit22.Text:='['+inttostr(ADOQuery3.FieldValues['uid'])+'] '+ADOQuery3.FieldValues['Пользователь'];
  if ADOQuery3.FieldValues['Дата ПК']<>null then edit25.Text:=ADOQuery3.FieldValues['Дата ПК'] else edit25.text:='Не указано';
  if ADOQuery3.FieldValues['Тип монитора']<>null then edit23.Text:=ADOQuery3.FieldValues['Тип монитора'] else edit23.text:='Не указано';
  if ADOQuery3.FieldValues['ШК монитора']<>null then edit17.Text:=ADOQuery3.FieldValues['ШК монитора'] else edit17.text:='Не указано';
  if ADOQuery3.FieldValues['IMEI монитора']<>null then edit24.Text:=ADOQuery3.FieldValues['IMEI монитора'] else edit24.text:='Не указано';
  if ADOQuery3.FieldValues['Дата монитора']<>null then edit26.Text:=ADOQuery3.FieldValues['Дата монитора'] else edit26.text:='Не указано';
  if ADOQuery3.FieldValues['Дата проектора']<>null then edit42.Text:=ADOQuery3.FieldValues['Дата проектора'] else edit42.text:='Не указано';
  if ADOQuery3.FieldValues['Тип проетора']<>null then edit29.Text:=ADOQuery3.FieldValues['Тип проетора'] else edit29.text:='-';
  if ADOQuery3.FieldValues['ШК проектора']<>null then edit28.Text:=ADOQuery3.FieldValues['ШК проектора'] else edit28.text:='';
  if ADOQuery3.FieldValues['IMEI проетора']<>null then edit27.Text:=ADOQuery3.FieldValues['IMEI проетора'] else edit27.text:='';

  if ADOQuery3.FieldValues['Тип проетора']<>'-' then begin
  ...
  Button7.Caption:='Сохранить';
  Button13.Caption:='Отмена';
end;
procedure TForm1.DBGrid3CellClick(Column: TColumn);
begin
  if gro_or_stud=0 then begin
    if ADOQuery3.FieldValues['Актив']=1 then CheckBox2.Checked:=true else CheckBox2.Checked:=false;
    if ADOQuery3.FieldValues['Служба']<>null then edit11.text:=ADOQuery3.FieldValues['Служба'] else edit11.text:='Не указано';
    if ADOQuery3.FieldValues['Код группы']<>null then edit12.text:=ADOQuery3.FieldValues['Код группы'] else edit12.text:='Не указано';
    if ADOQuery3.FieldValues['Программа']<>null then ComboBox3.text:=ADOQuery3.FieldValues['Программа'] else ComboBox3.text:='Не указано';
    if ADOQuery3.FieldValues['Профессия']<>null then edit15.text:=ADOQuery3.FieldValues['Профессия'] else edit15.text:='Не указано';
    if ADOQuery3.FieldValues['Начало практики']<>null then edit14.text:=ADOQuery3.FieldValues['Начало практики'] else edit14.text:='Не указано';
    if ADOQuery3.FieldValues['Конец практики']<>null then edit16.text:=ADOQuery3.FieldValues['Конец практики'] else edit11.text:='Не указано';
    if ADOQuery3.FieldValues['Дата заезда']<>null then edit13.text:=ADOQuery3.FieldValues['Дата заезда'] else edit13.text:='Не указано';
    if ADOQuery3.FieldValues['Дата выезда']<>null then edit33.text:=ADOQuery3.FieldValues['Дата выезда'] else edit33.text:='Не указано';
    if ADOQuery3.FieldValues['Дата явки']<>null then edit34.text:=ADOQuery3.FieldValues['Дата явки'] else edit34.text:='Не указано';
    if ADOQuery3.FieldValues['Куратор']<>null then ComboBox2.text:='['+inttostr(ADOQuery3.FieldValues['uid']) +'] '+ADOQuery3.FieldValues['Куратор'] else ComboBox2.text:='Не указано';
  end else begin
    if ADOQuery3.FieldValues['Профессия']<>null then edit15.text:=ADOQuery3.FieldValues['Профессия'] else edit15.text:='Не указано';
    if ADOQuery3.FieldValues['Фамилия']<>null then edit11.text:=ADOQuery3.FieldValues['Фамилия'] else edit11.text:='Не указано';
    if ADOQuery3.FieldValues['Имя1']<>null then edit12.text:=ADOQuery3.FieldValues['Имя1'] else edit12.text:='Не указано';
    if ADOQuery3.FieldValues['Отчетсво']<>null then edit36.text:=ADOQuery3.FieldValues['Отчетсво'] else edit36.text:='Не указано';
    if ADOQuery3.FieldValues['Дата рождения']<>null then edit14.text:=ADOQuery3.FieldValues['Дата рождения'] else edit14.text:='Не указано';
    if ADOQuery3.FieldValues['Образование1']<>null then ComboBox3.text:=ADOQuery3.FieldValues['Образование1'] else ComboBox3.text:='Не указано';
    if ADOQuery3.FieldValues['Дата зачисления']<>null then edit13.text:=ADOQuery3.FieldValues['Дата зачисления'] else edit13.text:='Не указано';
    if ADOQuery3.FieldValues['Дата отчисления']<>null then edit34.text:=ADOQuery3.FieldValues['Дата отчисления'] else edit34.text:='Не указано';
    if ADOQuery3.FieldValues['Участок']<>null then edit33.text:=ADOQuery3.FieldValues['Участок'] else edit33.text:='Не указано';
    if ADOQuery3.FieldValues['Стаж']<>null then edit37.text:=ADOQuery3.FieldValues['Стаж'] else edit37.text:='Не указано';
    if ADOQuery3.FieldValues['Разряд']<>null then edit16.text:=ADOQuery3.FieldValues['Разряд'] else edit16.text:='Не указано';
    if ADOQuery3.FieldValues['№ приказа']<>null then edit39.text:=ADOQuery3.FieldValues['№ приказа'] else edit39.text:='Не указано';
    if ADOQuery3.FieldValues['Адрес регистрации']<>null then edit38.text:=ADOQuery3.FieldValues['Адрес регистрации'] else edit38.text:='Не указано';
    if ADOQuery3.FieldValues['Комментарий']<>null then edit40.text:=ADOQuery3.FieldValues['Комментарий'] else edit40.text:='Не указано';
  end;
    Button15.Caption:='Сохранить';
end;
...
begin
  with form1 do begin
    if  gro_or_stud=0 then begin
      ...
      label10.Caption:='Фамилия:';
      label11.Caption:='Имя:';
      label44.Caption:='Отчетсво:';
      label13.Caption:='Дата рождения:';
      label12.Caption:='Образование:';
      label39.Caption:='Зачисления:';
      label43.Caption:='Отчисления:';
      edit36.Visible:=true;
      label38.caption:='Участок:';
      label15.Caption:='Разряд:';
      ...
      DBGrid3.Columns[0].FieldName:='Имя';
      DBGrid3.Columns[1].FieldName:='Участок';
      DBGrid3.Columns[2].FieldName:='Дата рождения';
      DBGrid3.Columns[3].FieldName:='Профессия';
      DBGrid3.Columns[4].FieldName:='Стаж';
      DBGrid3.Columns[5].FieldName:='Образование';
      DBGrid3.Columns[6].FieldName:='Дата зачисления';
      DBGrid3.Columns[7].FieldName:='Дата отчисления';
      DBGrid3.Columns[8].FieldName:='№ приказа';
      DBGrid3.Columns[9].FieldName:='Адрес регистрации';
      DBGrid3.Columns[10].FieldName:='Комментарий';
      ...
      '              st_raz as [Разряд],'+
      '              st_staj as [Стаж],'+
      '              ob_name as [Образование],'+
      '              ob_lil as [Образование1],'+
      '              st_ar as [Адрес регистрации], '+
      '              st_numpric as [№ приказа], '+
      '              st_dot as [Дата отчисления],'+
      '              st_dza as [Дата зачисления], '+
      '              st_kom as [Комментарий] '+
      '              from students, obraz '+
      '              where '+sel_gr_id_sea+' (st_obr=ob_id) order by st_fam, st_name, st_ot ');
      label46.Caption:='Совпадений: '+inttostr(ADOQuery3.RecordCount);
      gro_or_stud:=1;
      Button15.Caption:='Создать';
    end;
  end;
...
begin
  with form1 do begin
    ...
    Button15.Caption:='Создать';
    gro_or_stud:=0;
    label10.Caption:='Служба:';
    label37.Caption:='Профессия:';
    label44.Caption:='Куратор:';
    label11.Caption:='Код группы:';
    label13.Caption:='Начало практики:';
    label39.Caption:='Дата заезда:';
    label43.Caption:='Дата явки:';
    label12.Caption:='Программа:';
    label15.Caption:='Конец практики:';
    label38.Caption:='Дата выезда:';
    ...
    sql_s('select user_id, user_fam + '' '' + LEFT(user_name, 1)+ ''.'' +LEFT(user_ot, 1) as [Куратор] from users ');
    c:=ADOQuery1.RecordCount;
    ComboBox2.Clear;
    for i:=1 to c do begin
      if (ADOQuery1.FieldValues['user_id']<>7) and (ADOQuery1.FieldValues['user_id']<>8) then begin
      na:='['+inttostr(ADOQuery1.FieldValues['user_id'])+'] '+ADOQuery1.FieldValues['Куратор'];
    ...
    DBGrid3.Columns[1].FieldName:='Служба';
    DBGrid3.Columns[2].FieldName:='Код группы';
    DBGrid3.Columns[3].FieldName:='Программа';
    DBGrid3.Columns[4].FieldName:='Профессия';
    DBGrid3.Columns[5].FieldName:='Дата заезда';
    DBGrid3.Columns[6].FieldName:='Дата выезда';
    DBGrid3.Columns[7].FieldName:='Начало практики';
    DBGrid3.Columns[8].FieldName:='Конец практики';
    DBGrid3.Columns[9].FieldName:='Дата явки';
    DBGrid3.Columns[10].FieldName:='Куратор';
...
//  DBGrid3.Columns[11].Destroy;
    ...
    '              p.pro_lil as [Программа],'+
    '              g.gr_sluj as [Служба], '+
    '              g.gr_cod as [Код группы], '+
    '              g.gr_sprec as [Профессия], '+
    '              g.gr_dza as [Начало практики], '+
    '              g.gr_dvi as [Конец практики], '+
    '              g.gr_stpra as [Дата заезда], '+
    '              g.gr_enpra as [Дата выезда], '+
    '              g.gr_active as [Актив], '+
    '              g.gr_yavka as [Дата явки],'+
    '              u.user_id as [uid], '+
    '              u.user_fam + '' '' + LEFT(user_name, 1)+ ''.'' +LEFT(user_ot, 1) as [Куратор] '+
...
begin
  ...
  if (edit8.Text='') then ch:=1;
    if ch<>0 then  MessageBox(Form1.HANDle, PChar('Укажите все данные'), PChar('Ошибка!'), MB_OK + MB_ICONERROR) else begin
      s1:=GetToken(edit22.Text,'[',2);
      s2:=GetToken(s1,']',1);
      if s2='' then MessageBox(Form1.HANDle, PChar('Не верно указан пользователь'), PChar('Ошибка!'), MB_OK + MB_ICONERROR) else begin
        sql_s('select user_id from users where user_id='+s2+' ');
        if (ADOQuery1.FieldValues['user_id']=Null) then MessageBox(Form1.HANDle, PChar('Не верно указан пользователь'), PChar('Ошибка!'), MB_OK + MB_ICONERROR) else begin
          sql_s('select comp_id from comps where (comp_ip='''+edit8.text+''') and (comp_id<>'+inttostr(ADOQuery3.FieldValues['c_id'])+') ');
          if ADOQuery1.FieldValues['comp_id']<>Null then MessageBox(Form1.HANDle, PChar('Данный IP уже есть в базе'), PChar('Ошибка!'), MB_OK + MB_ICONERROR) else begin
            if Button7.Caption='Сохранить' then begin
              ...
              '                  '''+edit10.Text+''', '+
              '                  '+s2+') ');
              MessageBox(Form1.HANDle, PChar('Новый компьютер успешно добавлен!'), PChar('Внимание!'), MB_OK + MB_ICONASTERISK);
            end;
            for i := 17 to 26 do
            TEdit(FindComponent('Edit'+IntToStr(i))).Text:='';
            CheckBox1.Checked:=false;
...
var help2:String;
begin
  IF Edit7.Text<>'Поиск' Then Begin
    ...
    '             comp_ip as [IP], '+
    '             comp_corp as [Корпус], '+
    '             comp_login as [Имя ПК], '+
    '             comp_cab as [Кабинет], '+
    '             comp_pc_type as [Тип ПК], '+
    '             comp_pc_scpc as [ШК ПК], '+
    '             comp_pc_imei as [IMEI ПК], '+
    '             comp_pc_date as [Дата ПК], '+
    '             comp_proj_type as [Тип проетора], '+
    '             comp_proj_sc as [ШК проектора], '+
    '             comp_proj_date as [Дата проектора], '+
    '             comp_proj_imei as [IMEI проетора], '+
    '             comp_mon_type as [Тип монитора], '+
    '             comp_mon_sc as [ШК монитора], '+
    '             comp_mon_imei as [IMEI монитора], '+
    '             comp_mon_date as [Дата монитора], '+
    '             user_fam + '' '' + LEFT(user_name, 1)+ ''.'' +LEFT(user_ot, 1) as [Пользователь], '+
    '             user_id as [uid] '+
    '      From   comps с, users '+
    ...
    label35.Caption:='Совпадений: '+inttostr(ADOQuery3.RecordCount);
  End;
end;
...
begin
  IF Edit22.Text<>'' Then Begin
    ...
    '             user_fam + '' '' + LEFT(user_name, 1)+ ''.'' +LEFT(user_ot, 1) as [Имя] '+
    '      From users '+
    ...
    if (c<5) and (c<>0) then begin
      ...
      ListBox3.Clear;
      For I := 1 to  c do Begin
        na:='['+inttostr(ADOQuery1.FieldValues['uid'])+'] '+ADOQuery1.FieldValues['Имя'];
...
begin
  ...
  IF Edit30.Text<>'Поиск' Then Begin
...
Begin
  IF Edit9.Text<>'Поиск' Then Begin
  ...
  SQL_S2('Select    comp_ip as [IP], '+
      ...
      '             comp_proj_type as [Тип проетора], '+
      '             comp_proj_sc as [ШК проектора], '+
      '             comp_proj_imei as [IMEI проектора], '+
      '             comp_mon_type as [Тип монитора], '+
      '             comp_mon_sc as [ШК монитора], '+
      '             comp_mon_imei as [IMEI монитора], '+
      '             comp_mon_date as [Дата монитора], '+
      ...
      label33.Caption:='Совпадений: '+inttostr(ADOQuery3.RecordCount);
  end;
End;
